...
public class StudentService {
	 ...
	 * @return 대입 성공 시 true, 실패 시( == 꽉 찼을 때) flase
	 */
	 ...
	 * 
	 * @param index
	 * @return 조회된 학생 1명 또는 null
	 */
	public Student selectIndex(int index) {

		// java.lang.ArrayIndexOutOfBoundsException
		// -> 배열 인덱스 범위를 초과 (0 부터 배열 길이-1 까지)
	...
	public Student selectName(String name) {
		...
		// null이 아니면서 이름이 name과 일치하는 학생 찾기
	...
	public String selectMaxMin() {
		...
		for (Student s : students) {
			...
			if (maxAvg < avg) { // 기본 최댓값 보다 현재 평균이 더 큰 경우
				maxAvg = avg;
				maxStudent = s;
			}
			
			if (minAvg > avg) { // 기본 최댓값 보다 현재 평균이 더 작은 경우
				minAvg = avg;
				minStudent = s;
			}
		}
