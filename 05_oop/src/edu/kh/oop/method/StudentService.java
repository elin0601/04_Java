...
// 필드
	...
	private String studentInform(Student s) {
		...
		return String.format("%s / %s / %c / %s / %d / %d", Student.shcoolName, s.getStudentNumber(), s.getGender(),
	...
	/**
	 * Java 역량이 얼마나 증가 또는 감소했는지 정수로 입력 받고 학생의 Java 역량을 수정 단, 수정된 Java 역량은 최대값, 최소값
	 * 범위를 넘어가지 못하게함
	 * 
	 * ex) Java 역량이 120인 경우 100(최대값) Java 역량이 -30인 경우 0(최소값)
	 * 
	 * @param s : std1 또는 std2 (학생 객체 주소)
	 */
	...
	private String compareJava(Student s1, Student s2) {
		...
		String result = String.format("%s : %d / %s : %d \n", s1.getName(), s2.getJava(), s2.getName(), s2.getJava());  // 문자열, 정수, 문자열, 정수 순서
	...
	private String compareHtml(Student s1, Student s2) {
		...
		String resultHtml = String.format("%s : %d / %s : %d \n", s1.getName(), s2.getHtml(),  s2.getName(), s2.getHtml());
		if(s1.getHtml() > s2.getHtml()) {
			return resultHtml + s1.getName() + "의 점수가 더 높습니다.";
		} 
		
		if(s1.getHtml() < s2.getHtml()) {
			return resultHtml + s2.getName() + "의 점수가 더 높습니다.";
		} 
		
		return resultHtml + "점수가 같습니다.";
