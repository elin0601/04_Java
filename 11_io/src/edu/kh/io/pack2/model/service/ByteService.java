...
//			}
			...
			fos.write(content.getBytes()); // 쪼개져서 내보내졌지만 출력 후 합쳐짐
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * - FileOutputStream은 1 바이트씩 데이터를 입출력
		 * 
		 * - BufferedOutputStream / BufferedInputStream
		 *  -> 버퍼에 입출력 할 내용을 모아서 한 번에 입출력!
		 *  
		 *  --> 스트림을 이용하는 횟수가 적어짐
		 *  	 -> 성능, 시간 효율 up!!   
		 * */
		...
		try {
			...
			// 출력 방법 2 : String을 byte[] 변환 후 출력
			bos.write(content.getBytes()); // 쪼개져서 내보내졌지만 출력 후 합쳐짐
	...
	public void fileByteInput() {
		...
		try {
			...
			while(true) { // 무한 반복
				...
				// 읽어온 값을 StringBuilder 객체에 누적
				sb.append((char)value); //  char로 강제 형변환해서 글자 형태를 추가
			}
						
			System.out.println(sb); // 읽어온 내용 콘솔에 출력
	...
	public void fileCopy() {
		...
		try {
			...
			// 키보드 입력을 받기 위한 String 객체 생성
			br = new BufferedReader( new InputStreamReader(System.in) );
			
			// 경로 입력 받기
			
			System.out.print("파일 경로 입력 : ");
			String target = br.readLine(); // 입력 된 한 줄 읽어오기
