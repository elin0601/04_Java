...
public class ExceptionHandlingService1 {
	...
	/**
	 * 예외 처리 구문 활용 예시 1
	 * - 보통 정상 수행 되는데 가끔 예외가 발생
	 *  + if 문으로 처리하지 못하거나 애매할 때
	 */
	public void method3() {
	...
	/**
	 * catch 구문 여러개 작성하기 2 
	 * 1) throw : 예외를 강제 발생(던짐)
	 * 2) 예외 + 다형성
	 *  - catch문 여러개 작성 시 
	 *  	부모 예외를 초리하는 catch 문이 자식 예외도 모두 잡아서 처리
	...
	public void method5() {
		...
		do {
			...
			// 자식 예외를 부모 catch가 모두 잡아서 처리하기 때문에
...
//			}
	 ...
	 * - 모든 예외의 최상위 부모 클래스
	 * 
	 * - catch문에 Exception으로 예외를 받기도 하면
	 * 	 발생되는 모든 예외를 잡아서 처리할 수 있다
	 */
	public void method6() {
		
		try {
			// 각종 예외 강제 발생 (Illegal : 잘못된, Argument : 전달 인자
			//throw new IllegalArgumentException();
			
			// EOF : End Of File - 읽어들인 파일 내용이 끝남
			//throw new EOFEexception();
