...
// 2) 추가, 수정, 삭제, 검색 등 다양한 기능 구현되어 있음
// 3) 객체만 저장 가능
//		(기본 자료형은 안됨,,, 하지만 Wrapper 클래스 이용하면 된디!!)
...
public class ListService { 
	...
	// - 인덱스가 존재함
	//  -> 인덱스로 순서가 구분되기 때문에 중복 데이터 저장이 가능
	...
	public void method1() {
		...
		//						== object, String, Student 등 객체 자료형 등
		//							 자료형 가리지 않고 들어갈 수 있다
		...
		// 2. E get(int index) : List에서 index 번쨔 요소 반환 받기
		
		// 3. int size() : List에 저장된 요소(객체) 개수를 반환
		...
		for(int i = 0 ; i <list.size() ; i++) {
			...
			if(list.get(i) instanceof Integer) {
				...
				// -> 호환되지 않는 조건 피연산자 유형 Object 및 int
	...
	public void method2() {
		...
		strList.add("치킨");
		strList.add("보쌈");
		// strList.add(1234); // String이 아니라서 오류
		...
		// 4. void add(int index, E e) : index번째 중간 삽입
		strList.add(1, "찜닭");
		
		// 5. String toString() : List 에 저장된 모든 요소 문자열로 반환
		System.out.println(strList);
		// == System.out.println(strList.toString());
		
		// 6. E set(int index, E e) : index번째 요소를 e로 수정
		// -> 수정 전 요소가 반환됨
		String before = strList.set(2, "양념치킨");
		
		System.out.println("수정 전 : " + before); // 수정 전 : 치킨
		System.out.println("수정 후 : " + strList.get(2)); // 수정 후 : 양념치킨
		System.out.println(strList); // [닭갈비, 찜닭, 양념치킨, 보쌈, 족발, 카레돈까스, 한우불고기버거]
		
		
		// 7. int indexOf(E e) : List 내부에 e와 같은 요소가 있으면 해당 인덱스
		// 										  없으면 -1 반환
		System.out.println(strList.indexOf("보쌈")); // 3
		System.out.println(strList.indexOf("막국수")); // -1
		
		
		// 8. boolean contains(E e) :  List 내부에 e와 같은 요소가 있으면 true,
		//														 없으면 false 반환
		...
		// 9. E remove(int index) : index번째 요소를 제거(추출!)
		// 													-> 제거된 요소가 반환됨
		
		String target = strList.remove(0); // 0번 인덱스 요소를 제거
		System.out.println("제거된 요소 : " + target); // 제거된 요소 : 닭갈비
		System.out.println(strList); // [찜닭, 양념치킨, 보쌈, 족발, 카레돈까스, 한우불고기버거]
	}

	
	/**
	 * Book 으로 제한된 List 객체 생성힌 후
	 * - 추가, 조회, 수정, 제거 등 수현
	 */
	public void bookManage() {
		...
		List<Book> bookList = new LinkedList<Book>();  // 인터페이스 이용해 데이터 타입 변경이 쉬움
		...
		do {
			...
			sc.nextLine();  // 스케너 버퍼에 남아있는 개행 문자 제거
			...
			case 2 : 
				...
				// 향상된 for 문
				for(Book b :bookList) System.out.println(b); break;
